...
山河 VPC 网络一直支持虚拟 IP（Virtul_IP）的功能，只需要您采用支持虚拟 IP的软件 (如 keepalived) 就可以将 VPC 内的任何 IP 地址作为虚拟 IP 使用，即便未在山河控制台中指定创建虚拟 IP。
...
山河控制台提供了虚拟 IP 管理功能，用户创建虚拟 IP 后，将在控制台中展示出来虚拟 IP 被占用的情况。
