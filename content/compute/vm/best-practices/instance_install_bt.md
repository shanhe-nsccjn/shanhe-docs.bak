...
### 宝塔面板使用山河平台证书
用户在山河平台申请了SSL证书（[申请SSL证书步骤](https://docsv3.shanhe.com/security/ssl/manual/user_guide/)），需要在宝塔中部署的网站中使用
...
### 宝塔面板端口放行
用户通过宝塔面板部署了应用程序，部署后已按照放行8888端口的方式在山河平台放行了服务端口，却仍然无法正常访问服务，此时需要检查宝塔面板上的防火墙是否放行该服务端口。  
