...
1. 更新 容器集群服务QKE 参数：在 容器集群服务QKE 配置 Kubernetes EIP 地址
...
容器集群服务QKE 的 Kubernetes EIP 地址可以在 容器集群服务QKE 创建时指定，也可以在 容器集群服务QKE 运行时更新。运行时修改 容器集群服务QKE 的 Kubernetes EIP 地址会造成 Kubernetes Apiserver 重启，不建议频繁更新。配置后，此 EIP 地址会自动更新到 Apiserver 的 TLS 证书中。
...
对于 容器集群服务QKE 所在 VPC 绑定了 EIP 可以采用此方法。

找到访问 Kubernetes Apiserver 的私有网络 IP 地址。对于一个主节点的 容器集群服务QKE，到 容器集群服务QKE 详情页查询 容器集群服务QKE 主节点私有网络 IP 地址。

![](../../_images/access-kubernetes-vxnet-ip-single.png)

对于高可用的 容器集群服务QKE，访问 Kubernetes Apiserver 的私有网络 IP 地址应为 容器集群服务QKE 创建时创建的私有网络负载均衡器的 IP，负载均衡器名为 容器集群服务QKE 集群 ID。
...
> 该参数为 `v3.0.0 - KubeSphere v3.0.0` 版本新增参数，用来主动修改 kubeconfig。如果使用的是老版本 容器集群服务QKE，则只能使用默认的 `6443` 端口，不可修改。

![](../../_images/kubernetes-eport.png)

#### 通过负载均衡器

对于通过负载均衡器访问到 容器集群服务QKE Kubernetes Apiserver 的集群可以采用此方法。
...
在 Kubeconfig 标签页将 容器集群服务QKE 的 Kubernetes Kubeconfig 拷贝到用户本地云服务器，即可使用。
...
可在 shanhe 控制台 容器集群服务QKE 集群详情页的 "KubeSphere 控制台链接" 标签页找到 KubeSphere 控制台访问链接。
...
从 `容器集群服务QKE v1.0.1` 版本起内置了 [shanhe CSI](https://github.com/yunify/shanhe-csi)，可以动态创建基于 shanhe IaaS 上的硬盘的 PVC，并挂载到 Pod，当 Pod 迁移时，硬盘会自动随着 Pod 迁移到其他云服务器上。KubeSphere 的存储卷使用方法见[文档](https://kubesphere.com.cn/en/docs/installing-on-linux/introduction/storage-configuration/)。

### 使用 NeonSAN 硬盘

容器集群服务QKE 在支持 NeonSAN 硬盘的区创建了 neonsan 存储类型，用户可以在 KubeSphere 控制台创建 NeonSAN 存储卷。
...
容器集群服务QKE 预安装了 NFS 客户端程序，用户对接 NFS 服务端时应确保 容器集群服务QKE 各节点有权限挂载 NFS 服务端文件夹。用户可以使用 [Kubernetes 官方方法对接 NFS 服务端](https://kubernetes.io/docs/concepts/storage/volumes/#nfs)，这是一种静态分配存储卷方法，分配和回收存储卷过程复杂，可对接多个 NFS 服务端。为了方便用户对接 NFS 服务端，容器集群服务QKE 预置了 [NFS 动态分配器](https://github.com/helm/charts/tree/master/stable/nfs-client-provisioner)，支持动态分配存储卷，分配和回收存储卷过程简便，可对接一个 NFS 服务端。

> 示例 NFS 服务端 IP 为 192.168.0.4，NFS 共享文件夹为 /data。

### 安装 NFS 分配器

在 容器集群服务QKE client 节点，执行 Helm 安装命令
...
> `容器集群服务QKE v1.0.1` 及更早版本请执行下面的命令：
> ```
